...
export const useGameStore = create<GameState>((set, get) => ({
  ...
  buyBusiness: (businessType) => {
    ...
    const currentCount = user.businesses.find(b => b.type === businessType)?.count || 0;
    ...
    if (localCoins >= cost) {
      set((state) => {
        const updatedUser = {
          ...
          businesses: state.user!.businesses.map(b =>
    ...
    if (localCoins >= upgradeCost && !user.upgrades.some(u => u.type === upgradeType)) {
      set((state) => {
        const updatedUser = {
          ...
          upgrades: [...state.user!.upgrades, { id: Date.now().toString(), type: upgradeType }]
        };
        const newIncome = calculateIncome(updatedUser);
        return {
          user: updatedUser,
